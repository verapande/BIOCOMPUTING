...
To use my pipeline, you would first need to connect to the Bora cluster and navigate to your BIOCOMPUTING/assignments directory. Copy or clone the assignment_7 folder if needed and move into it. Once you are inside the project directory, make sure that all scripts in the scripts folder are executable by running chmod +x scripts/*.sh. If running the pipeline from a different folder name than mine, open the assignment_07_pipeline.slurm file and update the root variable so that it points to the correct directory path on your account. Once the path is confirmed, submit the pipeline to the scheduler using sbatch assignment_07_pipeline.slurm. This SLURM script will automatically run each step in the correct order: downloading SRA reads, cleaning the reads, mapping them to the dog genome using bbmap, extracting mapped reads with samtools. You will have to RUN SCRIPT 4 SEPARATELY. While the job runs, you can monitor progress using squeue -u $USER or watch output live using tail -f JOBNAME_*.out (replacing the * with the correct job ID if necessary). When the job finishes, check the data/raw folder for downloaded fastq files, the data/clean folder for cleaned reads, and the output folder for mapping results, mapped ID lists, and the final contamination summary table. The summary table will report total cleaned reads, mapped dog reads, and the approximate dog contamination percentage for each sample when you run the 4th script on its own separately (NOT IN PIPELINE). This pipeline is modular, so any individual script may be edited and rerun if needed, and it works towards a reproducible workflow.
<br>
<br>
<br>
## Reflection
