...
Task 7 — Compare the logs
The pipeline prints the last ten lines of the three logs. This is enough to quickly check whether Flye finished, what the final contig stats look like, and if any warnings differ across environments.

Task 8 — Build the pipeline.sh
My pipeline.sh calls the download script, the local build, the Conda installer, then the three runs, and finally the tail -n 10 summaries. I used strict mode (set -ueo pipefail) so errors surface early, but I allow the log tailing to continue even if a given run produced no file yet.

Task 9 — Delete everything except scripts and start over
To simulate grading, I removed assemblies/, data/, and flye-env.yml and ran the pipeline again. This design proved that the project is self-contained and reproducible from a clean state.
...
Data paths first. When Flye said it couldn’t open my FASTQ, the file simply wasn’t in ./data. I now let run scripts accept data/*.fastq* so an exact filename isn’t required.
...
Reflection (honest and grounded)
...
git push
If you are reading this in the future and need to run the work, start the VPN, SSH to bora, cd into this folder, and run bash pipeline.sh. Everything else is automated and documented above.
